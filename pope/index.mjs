...
import {
  ...
  loadNodePhysX,
  storage,
} from '../hyperfy/build/world-node-client.js';
import { AgentControls } from '../simple-agent/AgentControls.js';
import { AgentLoader } from '../simple-agent/AgentLoader.js';
import { PopeAgent } from './PopeAgent.mjs';
...
const CHAT_INTERVAL = 7000 // ms (how often the agent sends a chat message) - Updated for Pope to be less frequent
const LOG_INTERVAL = 1000 // ms (how often to log user data)
const ENTITY_LOG_INTERVAL = 5000 // ms (how often to log all entities)
...
let entityLogIntervalId = null
let playerNamesMap = new Map()
let appearanceIntervalId = null
let appearanceSet = false

const AGENT_NAME = 'Pope'
...
async function runAgent() {
  console.log(`Pope Agent connecting to ${WS_URL}...`)
  ...
  try {
    ...
    console.log("[Pope Agent Setup] Explicitly loading PhysX via loadNodePhysX...");
    await loadNodePhysX(); // Ensure PhysX is loaded and PHYSX is global
    console.log("[Pope Agent Setup] PhysX loaded, calling extendThreePhysX...");
    extendThreePhysX(); // <-- Call the extension function
    console.log("[Pope Agent Setup] extendThreePhysX called.");
    // -------------------------
    ...
    else { console.log("[Pope Agent Setup] Physics system seems initialized."); }
    // --------------------------

    // --- Load Environment Geometry ---
    console.log("[Pope Agent Setup] Attempting to load environment model...");
    const envModelUrl = world.settings?.model?.url;
    if (envModelUrl) {
       try {
           const envGltf = await world.loader.load('model', envModelUrl);
           console.log(`[Pope Agent Setup] Environment model loaded: ${envModelUrl}`);
           await setupStaticPhysicsGeometry(world, envGltf); // Call the setup function
       } catch (error) {
            console.error(`[Pope Agent Setup] Failed to load or process environment model ${envModelUrl}:`, error);
       }
    } else {
         console.warn("[Pope Agent Setup] No environment model URL found in world settings.");
    }
    // -------------------------------

    // --- Add listeners after world is ready ---
    if (world?.entities) { 
        console.log('[Pope Agent Setup] Attaching entityAdded/Removed listeners to world.entities...');
        world.entities.on('entityAdded', entityAddedListener);
        world.entities.on('entityRemoved', entityRemovedListener);
    }
    // -----------------------------------------------------------------

    // Initialize Pope agent with the world
    popeAgent = new PopeAgent(world, {
        popeBackendUrl: 'ws://localhost:8765',
        characterPath: './pope-config.json',
        debug: true
    });
    
    // Start the Pope agent
    await popeAgent.loadCharacter();
    await popeAgent.start();
    console.log('Pope agent started successfully');
    ...
    world.on('disconnect', reason => {
      console.log('Pope Agent Disconnected.', reason)
      stopAgent()
    })
    world.on('kick', code => {
      console.log('Pope Agent Kicked:', code)
      stopAgent()
    })
    ...
    console.error('Failed to initialize Pope agent:', error)
    stopAgent()
  }
}
...
function startSimulation() {
  ...
  function tickLoop() {
    ...
    try {
      ...
      console.error('[Pope Agent tickLoop] Error during world.tick:', e)
    }
    lastTickTime = now
  ...
  console.log(`[Pope Agent startSimulation] Starting simulation tick at ${TICK_RATE}Hz.`)
  tickLoop() // Start the first tick
}

function startRandomMovement() {
  if (moveIntervalId) clearInterval(moveIntervalId)
  console.log(`[Pope Agent startRandomMovement] Initializing interval every ${MOVE_INTERVAL}ms.`)
  ...
  moveIntervalId = setInterval(() => {
    ...
    if (!controls || typeof controls.setKey !== 'function') {
      console.error('[Pope Agent startRandomMovement] AgentControls system not found or missing setKey method!')
    ...
    // Popes move more deliberately - higher chance of stopping
    const direction = Math.floor(Math.random() * 6) // 0:W, 1:A, 2:S, 3:D, 4-5:Stop (increased stop probability)
    let moveKey = null

    switch (direction) {
      case 0:
        moveKey = 'keyW'
        console.log('Pope moving: FORWARD')
        break
      case 1:
        moveKey = 'keyA'
        console.log('Pope moving: LEFT')
        break
      case 2:
        moveKey = 'keyS'
        console.log('Pope moving: BACKWARD')
        break
      case 3:
        moveKey = 'keyD'
        console.log('Pope moving: RIGHT')
        break
      case 4:
      case 5:
        /* Stop */ console.log('Pope moving: STOP')
        break
    }
...
function startChatSubscription() {
  ...
  console.log('[Pope Agent startChatSubscription] Initializing chat subscription...')
  // Pre-populate processed IDs with existing messages
  world.chat.msgs?.forEach(msg => processedMsgIds.add(msg.id));
  console.log(`[Pope Agent startChatSubscription] Initial processed message IDs: ${processedMsgIds.size}`);
  ...
  world.chat.subscribe(msgs => {
    ...
    const agentPlayerName = world.entities.player.data.name || 'Pope'; // Get agent's name
    ...
    if (newMessagesFound.length > 0) {
      console.log(`[Pope Agent Chat] Found ${newMessagesFound.length} new messages to process.`);
      newMessagesFound.forEach(msg => {
        const senderName = msg.from || 'System'
        const messageBody = msg.body || ''
        console.log(`[Pope Agent Chat Received] From: ${senderName}, ID: ${msg.id}, Body: "${messageBody}"`);

        // Respond only to messages not from the agent itself
        if (msg.fromId !== agentPlayerId) {
          const response = `Peace be with you, ${senderName}. Regarding your message "${messageBody}", I offer my humble insights.`;
          console.log(`[Pope Agent Chat Sending] Body: "${response}"`);
          world.chat.add({ 
            body: response, 
...
function startRandomChatting() {
  ...
  console.log(`[Pope Agent startRandomChatting] Initializing interval every ${CHAT_INTERVAL}ms.`)

  const messagesToSend = [
    'Peace be with you in this digital realm.',
    'May digital grace and wisdom guide your path today.',
    'Faith and technology can work together to create a better world.',
    'Even in this virtual space, we can find moments of reflection and meaning.',
  ...
  chatIntervalId = setInterval(() => {
    ...
    console.log(`[Pope Agent Chat Sending Random] Body: "${randomMessage}"`);
...
function startUserDataLogging() {
  ...
  console.log(`[Pope Agent startUserDataLogging] Initializing interval every ${LOG_INTERVAL}ms.`)
...
function startEntityLogging() {
  ...
  console.log(`[Pope Agent startEntityLogging] Initializing interval every ${ENTITY_LOG_INTERVAL}ms.`);
  ...
  entityLogIntervalId = setInterval(() => {
    ...
    if (!world || !world.entities || !world.entities.items) {
      console.log("[Pope Agent Entity Logging] Waiting for world/entities...");
      return;
    }
    ...
    console.log(`
--- [Pope Agent Entity Log - Time: ${world.time.toFixed(2)}s] --- (${entityCount} entities) ---`);
    ...
    try {
      world.entities.items.forEach((entity, id) => {
        ...
        // Get name from map ONLY for players
        ...
        if (entity.base) {
             ...
             // Fallback to data if base isn't available
      ...
      console.log(`--- [End Pope Agent Entity Log] ---`);
    } catch (e) {
        console.error("[Pope Agent Entity Logging] Error during entity iteration:", e);
    }
...
async function stopAgent() {
  console.log('Stopping Pope agent...')
  
  // Stop the Pope agent if it exists
  if (popeAgent) {
    try {
      await popeAgent.stop();
      console.log('Pope agent stopped successfully');
    } catch (error) {
      console.error('Error stopping Pope agent:', error);
    }
    popeAgent = null;
  }
  ...
  if (logIntervalId) clearInterval(logIntervalId);
  if (entityLogIntervalId) clearInterval(entityLogIntervalId);
  if (appearanceIntervalId) clearInterval(appearanceIntervalId); appearanceIntervalId = null;
  tickIntervalId = null
  moveIntervalId = null
  chatIntervalId = null
  logIntervalId = null
  ...
  if (world?.livekit) {
    ...
    console.log('[Pope Agent] LiveKit disconnected.')
  }
  ...
  // Remove listeners and clear map before destroying world
  if (world?.entities) { 
      console.log("[Pope Agent Cleanup] Removing entity listeners from world.entities...");
  ...
  world?.destroy()
  world = null
  console.log('Pope agent stopped and resources released.');
}
...
// Helper Function to Set Appearance
function setAgentAppearance(targetWorld, name, avatarUrl) {
    ...
    console.log(`[Pope Agent Setup] Setting name to "${name}" and avatar to "${avatarUrl}" using network ID ${playerId}`);
...
async function uploadAndSetAvatar(agentPlayer) {
    ...
    try {
        ...
        console.log(`[Pope Agent Appearance] Reading avatar file from: ${filePath}`);
        ...
        if (!world.network || typeof world.network.upload !== 'function' || !world.assetsUrl) {
            console.error("[Pope Agent Appearance] world.network.upload function or world.assetsUrl not available!");
            return false; // Indicate failure
        }

        console.log(`[Pope Agent Appearance] Uploading ${fileName} (${(fileBuffer.length / 1024).toFixed(2)} KB)...`);

        // Hash the buffer directly
        console.log(`[Pope Agent Appearance] Constructing expected HTTP(S) URL...`);
        const hash = await hashFileBuffer(fileBuffer); // Hash the buffer
        if (!hash) {
             console.error("[Pope Agent Appearance] Failed to calculate file hash for URL construction.");
             return false;
        }
        ...
        console.log(`[Pope Agent Appearance] Constructed HTTP(S) URL: ${constructedHttpUrl}`);
        ...
        await world.network.upload(uploadData);
        console.log(`[Pope Agent Appearance] Upload process initiated.`);
        ...
        if (typeof agentPlayer.setSessionAvatar === 'function') {
            ...
            console.log(`[Pope Agent Appearance] Called setSessionAvatar with constructed URL: ${constructedHttpUrl}`);
            ...
            // Fallback: Try sending a network message
             ...
             console.log(`[Pope Agent Appearance] Sent playerSessionAvatar network message with: ${constructedHttpUrl}`);
             return true;
        } else {
            console.error("[Pope Agent Appearance] world.entities.player.setSessionAvatar method or network send not found!");
            return false; // Indicate failure
        }

    } catch (error) {
        if (error.code === 'ENOENT') {
            console.error(`[Pope Agent Appearance] Error: Avatar file not found at ${path.resolve(__dirname, LOCAL_AVATAR_PATH)}`);
        } else {
            console.error("[Pope Agent Appearance] Error during avatar upload/set process:", error);
        }
        return false; // Indicate failure
    }
}
...
function startAppearancePolling() {
    ...
    console.log(`[Pope Agent Appearance Polling] Initializing interval every 3000ms.`);

    appearanceIntervalId = setInterval(async () => {
        const agentPlayer = world?.entities?.player;
        const networkReady = world?.network?.id != null;

        if (!appearanceSet && agentPlayer && networkReady) {
            console.log(`[Pope Agent Appearance Polling] Player entity and network ready. Attempting to upload and set avatar...`);

            const success = await uploadAndSetAvatar(agentPlayer);
            ...
            if (success) {
                ...
                console.log(`[Pope Agent Appearance Polling] Avatar successfully set. Polling stopped.`);
            } else {
                console.log(`[Pope Agent Appearance Polling] Avatar setting failed, will retry...`);
...
// Physics Geometry Setup
async function setupStaticPhysicsGeometry(world, gltf) {
     console.log("[Pope Agent Physics] Starting setupStaticPhysicsGeometry...");
     ...
     if (!physics || !scene || !material) {
         console.error("[Pope Agent Physics] Physics system components (physics, scene, material) not ready.");
         return;
     }
     console.log("[Pope Agent Physics] PhysX components obtained.");
     ...
     gltf.scene.traverseVisible((node) => {
        if (node.isMesh && node.geometry) {
            console.log(`[Pope Agent Physics] Processing mesh: ${node.name || '(unnamed)'}`);
            meshesProcessed++;
            ...
            try {
                ...
                const meshScale = new PHYSX.PxMeshScale(node.scale.toPxVec3(), {x:0, y:0, z:0, w:1});
                ...
                console.error(`[Pope Agent Physics] Error processing mesh ${node.name || '(unnamed)'}:`, error);
     ...
     if (traversalErrors.length > 0) {
          console.warn(`[Pope Agent Physics] Finished setupStaticPhysicsGeometry. Meshes processed: ${meshesProcessed}, Actors added: ${actorsAdded}, Errors: ${traversalErrors.length}.`);
     } else {
        console.log(`[Pope Agent Physics] Finished setupStaticPhysicsGeometry. Meshes processed: ${meshesProcessed}, Actors added: ${actorsAdded}.`);
     }
}
